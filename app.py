...
class HedgeAssistantApp(tk.Tk):
    ...
    def _build_form(self) -> None:
        ...
        self._add_field(card, "Отрасль", self._build_dropdown(card, self.industry_var, INDUSTRIES), 0)
        self._add_field(card, "Валюта хеджирования", self._build_dropdown(card, self.currency_var, CURRENCIES), 1)
        ...
        button.grid(row=2, column=3, sticky="e", padx=20, pady=(0, 18))
    ...
    def _add_field(self, parent: tk.Widget, label: str, widget: tk.Widget, column: int) -> None:
        ...
        field.grid(row=1 if column > 2 else 0, column=column % 4, sticky="ew", padx=20, pady=(18, 8))
    ...
    def _on_submit(self) -> None:
        try:
            volume = float(self.volume_var.get().replace(" ", ""))
        except ValueError:
